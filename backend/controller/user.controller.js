...
const userCoverImageUpdate = async (req, res) => {
  ...
  const cloudinaryRes = await uploadOnCloudinary(coverImageLocalPath);
  if (!cloudinaryRes)
    return res
      .status(500)
      .json({ message: "Error uploading cover image", success: false });
  const coverImage = cloudinaryRes?.url;
...
const userProfileEdit = async (req, res) => {
  ...
  if (userName) {
    ...
    await userModel.findByIdAndUpdate(userId, { $set: { userName:newUserName } });
  }

  if (req.file?.path) {
    uploadOnCloudinary(req.file.path).then(async (res) => {
      await userModel.findByIdAndUpdate(userId, { $set: { image: res?.url } });
    });
  }
