...
const imageUpload = async (req, res) => {
  let { title, description, tags, id } = req.body;
  let imagePath = req.file?.path;

  if (!id) return res.status(400).send({ message: "Please Return Login" });

  // how to get owner id
  const token = jwt.verify(id, process.env.JWT_SECRET);
...
const singleUserProfile = async (req, res) => {
  ...
  if(!mongoose.isValidObjectId(userId)) return res.status(404).json({ message: "Invalid user id", success: false });
